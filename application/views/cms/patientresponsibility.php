...
j(document).ready(function(){
	search();
	j(".p_respons").addClass("active");
	j("#alert").delay(2000).fadeOut(300);
	j(".patient").addClass("active open");
	j(".td_text").slimscroll({
	  ...
	  j(".page").click(function () {
			  ...
			  if ((file = this.files[0])) {
				  ...
				  img.onload = function () {
					var ext = file.name.split('.').pop().toLowerCase();
					  ...
					  tp=file.type;                  
					  //alert("H="+h+"W="+w);
					  if(h < 300 && w >=690 || h >= 300 && w < 690 || h < 300 && w < 690){
	   ...
	   j("#Cancelbtn").click(function(){
		   ...
		   j("#formdetail").attr('action','<?php echo base_url();?>index.php/Insert/insert_responsibility');
	   });
	...
	{
		j("#formdetail").attr('action','<?php echo base_url();?>index.php/Update/update_responsibility');
		...
		{
			var img_url = "<?php echo base_url();?>data/uploads/patient/"+l[k];
				if(l[k]=="No image"){  }
				else{
	...
	function Delete(id,obj){
		...
		j.ajax({
			url:"<?php echo base_url();?>index.php/Delete/delete_responsibility",
			type:"post",
			data:{'id':id},
	...
	function delete_one(id,img,all){
		if(confirm("Are you sure! you want to delete It.."))
		j.ajax({
			url:"<?php echo base_url();?>index.php/Delete/singleimage_responsibility",
			type:"post",
			...
			success:function(data){
               ...
               j("#alert1").fadeIn('slow');
               j("#alert1").delay(2000).fadeOut(300);
			   window.location="<?php echo base_url();?>index.php/Admin/patientresponsibility";
            },
            error: function () {
            }
	...
	function change_aboutus(id,str)
	{
		if(confirm("Are you sure, you want to Change Status!.."))
			j.ajax({
				url:"<?php echo base_url();?>index.php/Status/responsibility",
				type:"post",
				data:{'id':id,'str':str},
				success:function(data){
		   ...
		   source: function (request, response) {
			  j.ajax({
				 ...
				 // contentType: "application/json; charset=utf-8",
					url: "<?php echo base_url();?>index.php/Search_Data/Getresponsibility",
	...
	function clickAction()
	{
		...
		{
			if(confirm("Are you sure, you want to delete It.."))
			j("#hfield").attr('action','<?php echo base_url();?>index.php/Delete_Data/delete_responsibility');
			j("#hfield").submit();
		}
		else if(t==""){
				j('#alert1').show();
				...
				<div style="height: 925px;margin: 0px;" class="tab-content">	
					<div class="row">
						<ul class="nav nav-tabs" style="margin:16px;margin-top:0px;">
							<li class="active" id="t1"><a href="#tab1-1" data-toggle="tab">View Patient Responsibility</a></li>
							<li class="" id="t2"><a href="#tab1-2" data-toggle="tab">Add Patient Responsibility</a></li>
							<li class="pull-right">
						...
						<div class="tab-content" style="margin:16px;">
										...
										<tbody id="tdata">
										  ...
										  <tr>
														   ...
														   <td style="text-align:center;  padding: 10px 0px;">
															   <?php if($row['image']!=""){ ?>
															   <img src="<?php echo base_url()?>data/uploads/patient/<?php echo $row['image'];?>" style="width:100%;height:90px;border:1px solid #D7DADC;margin:0px">
															   <?php  }else{?>
							...
							<form id="hfield" name="hfield" method="post" action="<?php echo base_url();?>index.php/Admin/patientresponsibility">
							...
							<div id="tab1-2" class="tab-pane">
								<form id="formdetail" name="formdetail" class="form-horizontal" method="post" action="<?php echo base_url();?>index.php/Insert/insert_responsibility" enctype="multipart/form-data">
									<div class="panel panel-default">
										<div class="panel-heading">
											<h4 class="panel-title">Add Patient Responsibility</h4>
										</div>
...
<form id="formh" name="formh" method="post" action="<?php echo base_url();?>index.php/Update/singleimage_responsibility" enctype="multipart/form-data">
