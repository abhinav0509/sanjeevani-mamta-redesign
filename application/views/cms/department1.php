...
function Edit(obj,id)
	{
		...
		{
			var img_url = "<?php echo base_url();?>data/uploads/department/"+l[k];
			if(l[k]=="No image"){  }
			else{
...
{
	j.ajax({
			url: "http://192.168.2.19:8084/MedicoHelpline/HospitalAllDoctors",
			data: {
			  lgKey: "K8Y3WK27EPLx8AeZoJaoBAq"
			},
			type: "post",
			dataType: "json",
			success: function(jsonResponse){
				var data = jsonResponse.hospitalDoctorList;
				if(jsonResponse.editError=="Success")
				{
				j('#head').html("");
				j('#head').append("<option value=''>Select Doctors</option>");          
				for(i=0;i<data.length;i++)
				{ 
					j('#head').append("<option value='"+data[i].uid+"'>"+data[i].fname+"</option>");
				}
				j('#doctors').html("");
				j('#doctors').append("<option value=''>Select Doctors</option>");          
				for(i=0;i<data.length;i++)
				{ 
					j('#doctors').append("<option value='"+data[i].uid+"'>"+data[i].fname+"</option>");
				}
				}
			},
			error: function() {
...
{
	j.ajax({
						url: "http://192.168.2.19:8084/MedicoHelpline/HospitalDepartment",
                        data: {
                          lgKey: "K8Y3WK27EPLx8AeZoJaoBAq",
						  department: j("#type").val()
                        },
                        type: "post",
                        dataType: "json",
                        success: function(jsonResponse){
							var id = jsonResponse.newDeptid;
							if(jsonResponse.editError=="Success")
							{
								j("#depid").val(id);
								j("#formdetail").submit();
...
{
	j.ajax({
						url: "http://192.168.2.19:8084/MedicoHelpline/HospitalDepartmentList",
                        data: {
                          lgKey: "K8Y3WK27EPLx8AeZoJaoBAq"
                        },
                        type: "post",
                        dataType: "json",
                        success: function(jsonResponse){
							var data = jsonResponse.departmentList;
							if(jsonResponse.editError=="Success")
							{
								j('#title').html("");
								j('#title').append("<option value=''>Select Department</option>");          
								for(i=0;i<data.length;i++)
								{ 
									j('#title').append("<option value='"+data[i].id+"'>"+data[i].deptname+"</option>");
								}
								j('#sub_mnu1').html("");
								j('#sub_mnu1').append("<option value=''>Select Department</option>");          
								for(i=0;i<data.length;i++)
								{ 
									j('#sub_mnu1').append("<option value='"+data[i].id+"'>"+data[i].deptname+"</option>");
								}
							}
						},
						error:function(error){
								 ...
								 <div class="table-responsive"> 
										...
										<div class="col-sm-12" style="padding-bottom: 10px;">
										  <div class="form-group">                                            
												<div class="col-sm-3">
													...
													<select class="form-control" name="sub_mnu1" id="sub_mnu1" required>
														...
														<?php foreach($dept as $dp){?>
															<option value="<?php echo $dp['dept_id'];?>"><?php echo $dp['name']?></option>
														<?php }?>                                                  
													  </select>
												</div>
												<!--<div class="col-sm-3">
													<select class="form-control" name="sub_mnu2" id="sub_mnu2" required>
												...
												<div class="col-sm-3" style="display:none;">
													<input type="text" id="doc" name="doc" class="form-control" placeholder="Search By Doctors Name" />
												</div>
												<div class="col-sm-3">
									...
									<table id="" class="table table-bordered" style="float:left;">
										<thead style="background: #f5f5f5; color: #FFF;">  
											  ...
											  <!--<th width="20%" style="text-align:center;">Doctor's</th>-->
										...
										<tbody id="tdata">
										  <?php 
										  //$lines = file_get_contents('http://192.168.2.19:8084/MedicoHelpline/HospitalAllDoctors?lgKey=K8Y3WK27EPLx8AeZoJaoBAq&dataType=json');
										  //$data = json_decode($lines,true);
										  //print_r($data['hospitalDoctorList']);
										  if(!empty($result)){
										  ...
										  <tr>
											   ...
											   <td style="text-align:center;"><div class="tooltip"><?php echo $row['name'];?>
											   <!--<span class="tooltiptext"><?php echo $row['pdesc'];?></span>--></div>
											   </td>
											   ...
											   <td style="text-align:center;">
											    <?php if($row['image']!=""){?>
												<img src="<?php echo base_url()?>data/uploads/department/<?php echo $row['image'];?>" style="width:90px;height:90px;border:1px solid #D7DADC;margin:0px">
												<?php } else {?>
