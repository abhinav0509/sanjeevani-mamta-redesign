...
j(document).ready(function(){
	search();
	search1();
	j(".specialities").addClass("active open");
	...
	j(".td_text").slimscroll({
	  ...
	  j(".page").click(function () {
			  ...
			  if ((file = this.files[0])) {
				  ...
				  img.onload = function () {
					var ext = file.name.split('.').pop().toLowerCase();
					  ...
					  tp=file.type;                  
					  //alert("H="+h+"W="+w);
					  if(h < 200 && w >=400 || h >= 200 && w < 400 || h < 200 && w < 400){
						  alert("Please Upload Image In (400(Width) * 200(Height)) Dimentions");
						  j("#upload").val("");
						  j("#img1").attr('src','');
						  j("#img1").html("");
					  }
	   ...
	   j("#Cancelbtn").click(function(){
		   ...
		   j("#formdetail").attr('action','<?php echo base_url();?>index.php/Insert/insert_speciality');
	   });
...
function Edit(obj,id)
	{
		j("#formdetail").attr('action','<?php echo base_url();?>index.php/Update/update_speciality');
		...
		{
			var img_url = "<?php echo base_url();?>data/uploads/speciality/"+l[k];
				if(l[k]=="No image"){  }
				else{
	...
	function Delete(id,obj){
		...
		j.ajax({
			url:"<?php echo base_url();?>index.php/Delete/delete_speciality",
			type:"post",
			data:{'id':id},
	...
	function delete_one(id,img,all){
		if(confirm("Are you sure! you want to delete It.."))
		j.ajax({
			url:"<?php echo base_url();?>index.php/Delete/singleimage_speciality",
			type:"post",
			...
			success:function(data){
               ...
               j("#alert1").fadeIn('slow');
               j("#alert1").delay(2000).fadeOut(300);
			   window.location="<?php echo base_url();?>index.php/Admin/speciality";
            },
            error: function () {
	...
	function change_aboutus(id,str)
	{
		if(confirm("Are you sure, you want to Change Status!.."))
			j.ajax({
				url:"<?php echo base_url();?>index.php/Status/speciality",
				type:"post",
				data:{'id':id,'str':str},
				success:function(data){
		   ...
		   source: function (request, response) {
			  j.ajax({
				 ...
				 // contentType: "application/json; charset=utf-8",
					url: "<?php echo base_url();?>index.php/Search_Data/Getspeciality",
	...
	j("#sub_mnu2").autocomplete({
		   source: function (request, response) {
			  j.ajax({
				  type: "POST",
				 // contentType: "application/json; charset=utf-8",
					url: "<?php echo base_url();?>index.php/Search_Data/Getspectitle",
	...
	function clickAction()
	{
		...
		{
			if(confirm("Are you sure, you want to delete It.."))
			j("#hfield").attr('action','<?php echo base_url();?>index.php/Delete_Data/delete_speciality');
			j("#hfield").submit();
		}
		else if(t==""){
				j('#alert1').show();
				...
				<div style="height: 925px;margin: 0px;" class="tab-content">	
					<div class="row">
						<ul class="nav nav-tabs" style="margin:16px;margin-top:0px;">
							<li class="active" id="t1"><a href="#tab1-1" data-toggle="tab">View Speciality</a></li>
							<li class="" id="t2"><a href="#tab1-2" data-toggle="tab">Add Speciality</a></li>
							<li class="pull-right">
						...
						<div class="tab-content" style="margin:16px;">
										...
										<tbody id="tdata">
										  ...
										  <tr>
											   ...
											   <td style="text-align:center;  padding: 10px 0px;">
												   <?php if($row['image']!=""){?>
														<img src="<?php echo base_url()?>data/uploads/speciality/<?php echo $row['image'];?>" style="width:90px;height:90px;border:1px solid #D7DADC;margin:0px">
												   <?php } else{?>
											   ...
											   <td>
												   <select name="status" id = "status" class="form-control" onchange="change_aboutus('<?php echo $row['s_id'];?>',this.value)">
							...
							<form id="hfield" name="hfield" method="post" action="<?php echo base_url();?>index.php/Admin/speciality">
							...
							<div id="tab1-2" class="tab-pane">
								<form id="formdetail" name="formdetail" class="form-horizontal" method="post" action="<?php echo base_url();?>index.php/Insert/insert_speciality" enctype="multipart/form-data">
									<div class="panel panel-default">
										<div class="panel-heading">
											<h4 class="panel-title">Add Speciality Description</h4>
										</div>
										...
										<div class="col-sm-6">
											...
											<div class="form-group" style="display:none;" id="add1">
												<label class="col-sm-4 control-label" for="inputEmail3">Type <span class="asterisk">*</span></label>
												<div class="col-sm-8">
													<input type="text" placeholder="Add New Speciality" id="type" name="type" class="form-control">
												</div>
											</div>
											<div class="form-group">
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													...
													<span class="asterisk">Upload Image in (400 X 200) Dimention and allowed file Types are (jpg ,jpeg ,png)</span>
												</div>
											</div>
										</div>
										<div class="col-sm-6">
...
<form id="formh" name="formh" method="post" action="<?php echo base_url();?>index.php/Update/singleimage_speciality" enctype="multipart/form-data">
